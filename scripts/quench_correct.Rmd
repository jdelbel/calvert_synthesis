...
sb_correct %>% 
  filter(pres < 50) %>% 
  ggplot() +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 0, ymax = 6, fill = 'orange') +
  # geom_line(aes(x = flu_cast_cor, y = pres),
  #           orientation = "y",
  #           color = "forestgreen",
  #           size = 2) +
  geom_line(aes(x = flu_cor, y = pres),
            orientation = "y",
            color = "black",
            size = 2) +
  geom_line(aes(x = qc, y = pres),
            orientation = "y",
            color = "blue",
            size = 3) +
  geom_line(data = . %>%  filter(pres < 7), aes(x = flu_cast_cor, y = pres),
            orientation = "y",
            color = "blue",
            size = 3,
            linetype = "dotted") +
  geom_line(aes(x = chl_loess, y = pres),
            orientation = "y",
            color = "red",
            size = 2) +
  ...
  labs(x = bquote(CTD[FLU]~"(mg" ~ m^-3*")"),
       y = "Depth") +
  theme_bw() +
  theme(text = element_text(size = 40), #35
        axis.text = element_text(color = "black")) 

ggsave(here("figures", "quench_sb_correct_4.png"),
...
sb_correct %>% 
  ...
  scale_y_reverse(breaks = seq(0, 50, by = 10)) +
  facet_wrap(~date, scales = "free_x") +
  labs(x = bquote(CTD[PAR]~"(μmol" ~ m^-2~m^-1*")"),
       y = "Depth") +
  theme_bw() +
  theme(text = element_text(size = 40), #35
        axis.text = element_text(color = "black"))

ggsave(here("figures", "quench_sb_correct_par.png"),
        width = 9, height = 12, dpi = 300)
```
...
ggsave(here("figures", "quench_scm_correct.png"),
        width = 9, height = 12, dpi = 300)
```
...
```{r}
write.csv(ctd_slope, here("outputs", "quench_all_data_2024.csv"))
```
