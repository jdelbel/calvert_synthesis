...
chl <- read_csv(here("files", "buoy_chl_all_hourly.csv"))
...
chl_22 <- chl %>% 
  ...
  mutate(measurementTime_2 = lubridate::mdy_hm(measurementTime),
    ...
    mutate(group = case_when(date < "2021-08-04" ~ 1,
                             date > "2021-09-13" ~ 2)) %>% 
...
chl_22_q1 %>% 
  ...
  geom_line() +
  geom_point(data = filter(chl_22_q1, gf1 < -1), color = "red")
```
...
ggsave(here("figures", "buoy_2021_correction.png"), fig,
...
join_test %>% 
  ...
  ggpubr::stat_cor(data = filter(join_test, chla < 5),
                   ...
                   p.accuracy = 0.001, size = 9, label.y.npc = 0.6, color = "blue") +
  ggpubr::stat_regline_equation(data = filter(join_test, chla < 5),
                                size = 9, label.y.npc = 0.7, color = "blue") +
  lims(x = c(0, 20),
       y = c(0, 20)) +
  labs(x = "Chla",
       y = "Fluorescence") +
...
ggsave(here("figures", "scatter_2021.png"), 
        width = 8, height = 6, dpi = 300)
```

```{r}
write_csv(chl_22_q4, here("outputs", "buoy_corr_dm_2021.csv"))
```
