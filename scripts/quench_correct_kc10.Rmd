...
par_c <- read_csv(here("files_big", "8_binAvg-1743715163197.csv")) 
...
c_join <- c_dm %>% 
  ...
  mutate(chl_comb = case_when(is.na(chl_dm) ~ sum,
                              !is.na(chl_dm) ~ chl_dm)) %>% 
 mutate(chl_comb = case_when(date == "2023-05-14" & pres == 20 ~ sum,
                              TRUE ~ as.numeric(chl_comb))) %>%   
...
```{r}
write.csv(c_join, here("outputs", "chl_combined_fzh_kc10.csv"))
```
...
ggsave(here("figures", "insig_test_scatter_fix3.png"), 
        width = 16, height = 16, dpi = 300)
...
f_slope_quench <- f_slope_join %>% 
  # drop_na() %>% #I think this is an issue.
  filter(!is.na(slope_best)) %>%
...
test_data <- f_slope_quench 

correct_npq_single <- function(data) {
  # Objective function
  objective_func <- function(params, depth, fluor, chl_a) {
    npq_factor <- exp(params[1] * exp(-depth/params[2]))
    corrected_fluor <- fluor * npq_factor
    sum((corrected_fluor - chl_a)^2, na.rm = TRUE)
  }
  
  # Remove NA values for optimization
  clean_data <- data %>% 
    filter(!is.na(depth), !is.na(fluorescence_raw), !is.na(chl_a_concentration))
  
  if(nrow(clean_data) < 3) {
    # Return original data with uncorrected fluorescence if insufficient data
    return(data %>% 
           mutate(fluorescence_corrected = fluorescence_raw,
                  npq_param1 = NA,
                  npq_param2 = NA,
                  convergence = 1))
  }
  
  # Optimize using only clean data
  result <- optim(c(0.5, 10), objective_func,
                  depth = clean_data$depth,
                  fluor = clean_data$fluorescence_raw,
                  chl_a = clean_data$chl_a_concentration,
                  method = "L-BFGS-B",
                  lower = c(-2, 1),
                  upper = c(2, 100))
  
  # Apply correction to ALL data points (including those with NAs)
  corrected_data <- data %>%
    mutate(
      npq_factor = exp(result$par[1] * exp(-depth/result$par[2])),
      fluorescence_corrected = fluorescence_raw * npq_factor,
      npq_param1 = result$par[1],
      npq_param2 = result$par[2],
...
# Apply to your data
your_data <- f_slope_quench2 %>%
  filter(date == "2024-04-12") %>% 
  select(depth = pres, fluorescence_raw = f_slope, chl_a_concentration = chl_comb) 

# Calculate corrected fluorescence
corrected_profile <- correct_npq_single(your_data)
...
corrected_profile %>%
  filter(depth < 31) %>% 
  ggplot(aes(y = depth)) +
  geom_line(aes(x = fluorescence_raw), orientation = "y", color = "black") +
  geom_line(aes(x = fluorescence_corrected), orientation = "y", color = "red") +
  geom_point(aes(x = chl_a_concentration), color = "green") +
  scale_y_reverse()
```
...
interp <- interp %>% 
  mutate(pres = x.new) %>% 
  pivot_longer(cols = (1:21), names_to = "id", values_to = "chl_loess") %>% 
...
```{r}
#So care needs to be taken in using the ratio in situations with SCM and potential CTD bottle mismatch.
f_slope_join %>% 
...
ggsave(here("figures", "check_npq_prof_2023-05-14_20-fix.png"),
...
dup_list <- kc_fz_dup$id
```

```{r}

corrected %>% 
  filter(id %in% dup_list) %>%
  filter(pres < 51) %>% 
...
#Initial comparison shows that they can be different
```

```{r}
write.csv(corrected, here("outputs", "corrected_flu_2025-05-09.csv"))
```
