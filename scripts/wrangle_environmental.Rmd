...
wind <- read_csv(here("files_big", "2025-04-14.1hourSamples_lookout.csv"))

#Nutrients
n <- read_csv(here("files", "2025-04-14_HakaiData_nutrients.csv"))
...
```{r}
#I was going to join everything together into a CTD file, but now I am second guessing that as I will likely pull out specific metrics to correlate.
```
...
fw_dm <- fw %>% 
  ...
  ungroup() %>% 
  filter(n == 4) %>%
  select(date, wg = Watershed_group, q = Q) %>% 
  pivot_wider(names_from = "wg", values_from = "q") %>% 
  mutate(qt_b1_10 = rollmeanr(lag(Q_Total, 1), k = 10, fill = NA),
         qgm_b1_10 = rollmeanr(lag(Q_GM, 1), k = 10, fill = NA),
         qsm_b1_10 = rollmeanr(lag(Q_SM, 1), k = 10, fill = NA),
         qra_b1_10 = rollmeanr(lag(Q_RA, 1), k = 10, fill = NA)) 
```
