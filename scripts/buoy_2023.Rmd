...
chl <- read_csv(here("files", "2025-05-16.1hourSamples.2023_2.csv"))
...
chl_23 <- chl %>% 
  ...
  mutate(group = case_when(date >= "2023-04-02" ~ 2,
                           date <= "2023-03-29" ~ 1)) %>% 
  filter(date <= "2023-03-29" |
                           date >= "2023-04-02") %>% 
  filter(date < "2023-05-07")
...
# f1 <- 
  ...
  chl_23 %>% 
  ggplot(aes(measurementTime_2, fl, color = as.factor(group))) +
  geom_line() +
  labs(y = "Fluorescence",
       color = NULL) +
  theme_bw() +
...
chl_23 %>% 
  ...
  geom_line() +
  geom_point(data = filter(chl_23, gf < -1), color = "red")
```
...
# f3 <- 
  ...
  geom_line() +
  geom_point(data = filter(chl_23_q3, sd_mean_all > 3), color = "red") +
...
chl_23_q5 %>%
  left_join(cd) %>% 
  ggplot() +
  geom_line(aes(date_corr, fl_med_day, color = as.factor(group)), size = 2) +
...
```{r}
write_csv(chl_23_q5, here("outputs", "buoy_corr_dm_2023.csv"))
```
