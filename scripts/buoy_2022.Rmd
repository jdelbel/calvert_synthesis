...
chl_22_q6 %>% 
  ...
  geom_line() +
  geom_point(data = filter(chl_22_q6, gf6 < -1), color = "red")
```
...
f3 <- chl_22_q7 %>% 
  ggplot(aes(measurementTime_2, fl)) +
  geom_line() +
  geom_point(data = filter(chl_22_q7, sd_mean_all > 3), color = "red") +
...
ggsave(here("figures", "buoy_2022_correction.png"), fig,
...
```{r}
write_csv(chl_22_q9, here("outputs", "buoy_corr_dm_2022.csv"))
```
