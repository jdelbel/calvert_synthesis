...
ctd <- read.csv(here("files_big", "8_binAvg-1734114948350.csv"))
...
ggsave(here("figures", "prof_check_176-200_dfo2.png"), 
...
#Printing for easier visualization - just make sure to re-name with appropriate numbers
ggsave(here("figures", "prof_check_201-225_fzh01.png"), 
       width = 16, height = 16, dpi = 300)
...
#5168
#Above 80217?
#7482 - very odd profile
#9029 - clear issue
#23180 - strong downspikes - through further inspection this does not look good and needs to be removed.
...
#5074
#5173
#these must be 80217? Check.
#8406 has some negative spikes - will keep it for now as looks ok.
#9403 saturated
```

```{r}
#All of these are from 80217 and will be removed from
check_dfo <- prof_dfo2 %>% 
  filter(castpk == 4990 | castpk == 5074 | castpk == 5173) %>% 
  distinct(ctdNum, date)

#All of these are from 80217
check_fzh01 <- prof_fzh01 %>% 
  filter(castpk == 3827 | castpk == 4984 | castpk == 5107 | castpk == 5168) %>% 
  distinct(ctdNum, date)
```
...
ggsave(here("figures", "dark_correction_mean_minimum_10_all_ctds_kc10-fzho1-dfo2_rem_80217.png"), 
       width = 16, height = 16, dpi = 300)
...
#Once I added DFO2 and FZH01, it looks like DFO2 generally has the lowest dark values, but they are relatively comparable. 

#Exceptions:
# 2014 18066 KC10 and FZH01 casts- investigate. Happening at both stations - sort sort of sinking event and maybe offsets not representative?
#2024 - 18032 with very low offset - plot profile.


```

```{r}
#Changing tack a bit as it's hard to match offsets to stations that didn't go deep enough and were collected on different days. Thinking I can do a monthly average for each sensor and use that.

prof_10_fl %>% 
  mutate(month = lubridate::month(date)) %>% 
  group_by(ctdNum, station, year, month) %>% 
  summarize(min_mean_month = mean(min_mean),
            min_std_month = sd(min_mean)) %>%
  ungroup() %>% 
  mutate(day = 15) %>%
  group_by(ctdNum, year, month) %>% 
  filter(min_rank((min_mean_month)) == 1) %>% 
  ungroup() %>% 
  unite(date, c("year", "month", "day"), sep = "-") %>% 
  mutate(date = lubridate::ymd(date)) %>% 
  ggplot(aes(x = date, y = min_mean_month, color = station)) +
  geom_point(size = 3, pch = 21, fill = "white", stroke = 1.2) +
  geom_errorbar(aes(ymin = min_mean_month - min_std_month,
                    ymax = min_mean_month + min_std_month)) + 
  facet_wrap(~ ctdNum, ncol = 1, scales = "free_y") +
  labs(x = "Date",
           y = "Profile Avg. 10 minimum fluorescence values") +
  scale_y_continuous(limits = c(0, NA)) +
...
ggsave(here("figures", "dark_correction_mean_minimum_10_monthly_average.png"), 
       width = 16, height = 16, dpi = 300)
```

```{r}
#I feel like the monthly mean from station with the lowest value is probably the best. Can export it out and then apply in a different workbook. 
month_low <- prof_10_fl %>% 
  mutate(month = lubridate::month(date)) %>% 
  group_by(ctdNum, station, year, month) %>% 
  summarize(min_mean_month = mean(min_mean),
            min_std_month = sd(min_mean)) %>%
...
check_18066 <- prof_10_fl_means %>% 
  filter(ctdNum == 18066 & date < "2015-01-01")

prof %>% 
  filter(castpk == 2646 | castpk == 2645 | castpk == 2676 | castpk == 2681) %>% 
  filter(pres > 100) %>% 
  ggplot(aes(x = flu, y = pres, color = station)) +
  geom_line(orientation = "y", size = 2) +
  scale_y_reverse() +
  facet_wrap(~castpk, scales = "free") +
  lims(x = c(0, 0.4))

#Not really sure on these casts...
```
```{r}
check_18032 <- prof_10_fl_means %>% 
  filter(ctdNum == 18032 & date > "2020-01-01")

#I would say this is a bad cast. Very negative downspikes.
prof %>% 
  filter(castpk == 23180) %>% 
  filter(pres > 100) %>% 
  ggplot(aes(x = flu, y = pres, color = station)) +
...
```{r}
write.csv(prof_qc1_join, here("outputs", "kc10_fzh01_dfo2_profs_qc1_2024-12-18.csv"))

write.csv(month_low, here("outputs", "offset_low_month_mean.csv"))
```
