...
#I'm not sure what this is and I need to see where it is used.
...
```{r}
#Pulling out 2022 profiles from the corrected CTD data - This isn't needed here I dont think - I think I do this so I can pull in the interpolated profiles elsewhere.
f22 <- f %>% 
  mutate(year = year(date)) %>% 
  filter(year == 2022)
```
...
#It would probably be less verbose if I just joined this within in single dataframe earlier
```
...
```{r}
#Creating the 2024 wind speed cubed plot
f1_24 <-   
  ggplot() + 
...
```{r}
#This makes a plot with wind, freshwater and buoy data with 2022 and 2024 side by side. Not super useful, but does show how different each year is.
...
```{r}
#Plot of the full CHEMTAX time-series
chem_tchla %>%
  ggplot() +
...
```{r}
# Option 1: Custom gradient emphasizing bloom threshold
# Dark blue -> light blue -> green -> yellow -> orange -> red
fluor_palette_v1 <- colorRampPalette(c(
  "#08306b",  # Dark blue (low values)
  "#2171b5",  # Medium blue
  "#4292c6",  # Light blue (approaching bloom)
  "#41ab5d",  # Green (bloom onset ~5 μg/L)
  "#78c679",  # Light green
  "#c2e699",  # Yellow-green
  "#fed976",  # Yellow (moderate bloom)
  "#fd8d3c",  # Orange (high bloom)
  "#e31a1c"   # Red (very high bloom)
))

# Option 2: Oceanographic-inspired palette
# Deep ocean blue to surface bloom colors
fluor_palette_v2 <- colorRampPalette(c(
  "#08306b",  # Deep blue (oligotrophic)
  "#2171b5",  # Blue
  "#4292c6",  # Medium blue
  "#6baed6",  # Light blue (bloom threshold)
  "#9ecae1",  # Very light blue
  "#c6dbef",  # Pale blue
  "#fee391",  # Pale yellow
  "#fec44f",  # Yellow
  "#fe9929",  # Orange
  "#d95f0e"   # Dark orange (intense bloom)
))
...
colors_v1 <- fluor_palette_v1(50)
colors_v2 <- fluor_palette_v2(50)
...
```{r}
test2 <- ggarrange(f1_22, f2_22, f3, f4, ncol = 1)



ggsave(here("figures", "panel_kc10_2022_CHEMTAX_3.png"), test2,
