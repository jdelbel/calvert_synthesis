...
chl_22_qc <- chl_22 %>% 
  group_by(group) %>% 
  mutate(
    gf = c(NA, diff(fl)/lag(fl)[-1]),  # Fix GF calculation
    # Outlier flags
    fl_outlier = detect_outliers_iqr(fl, k = 2),
    gf_outlier = detect_outliers_iqr(abs(gf), k = 1.5),
    # Combined flag
    outlier_flag = fl_outlier | gf_outlier | fl < 0 | fl > 50
  ) %>%
  filter(!outlier_flag) %>%
  ungroup()
...
chl_22_qc_lo <- chl_22 %>% 
  group_by(group) %>% 
  arrange(measurementTime_2) %>%  # assuming you have a datetime column
  mutate(
    # Rolling mean and SD (adjust window size as needed)
    roll_mean = rollmean(fl, k = 5, fill = NA, align = "center"),
    roll_sd = rollapply(fl, width = 5, FUN = sd, fill = NA, align = "center"),
    # Z-score relative to local conditions
    local_zscore = abs(fl - roll_mean) / roll_sd,
    # Growth factor
    gf = c(NA, diff(fl)/lag(fl)[-1]),
    # Flag extreme values
    spike_flag = local_zscore > 3 | abs(gf) > 1 | fl < 0 | fl > 50
  ) %>%
  filter(!spike_flag) %>%
  ungroup()
...
chl_22_cpd  <- chl_22 %>% 
  ...
  mutate(
    # First derivative (growth factor)
    gf = (fl - lag(fl)) / lag(fl),
    # Second derivative (change in growth factor)
    gf2 = gf - lag(gf),
    # Adaptive thresholds
    gf_threshold = quantile(abs(gf), 0.95, na.rm = TRUE),
    gf2_threshold = quantile(abs(gf2), 0.95, na.rm = TRUE),
    # Flag spikes
    spike_flag = abs(gf) > gf_threshold | 
                 abs(gf2) > gf2_threshold | 
                 fl < 0 | 
                 fl > 50
  ) %>%
  filter(!spike_flag) %>%
...
clean_fluorometry <- function(data, max_iter = 5) {
  for(i in 1:max_iter) {
    n_before <- nrow(data)
    ...
    data <- data %>%
      ...
      mutate(
        gf = c(NA, diff(fl)/lag(fl)[-1]),
        gf_mad = mad(gf, na.rm = TRUE),  # Median absolute deviation
        gf_median = median(gf, na.rm = TRUE),
        outlier = abs(gf - gf_median) > 3 * gf_mad | fl < 0 | fl > 50
      ) %>%
      filter(!outlier) %>%
    ...
    cat(sprintf("Iteration %d: removed %d points\n", i, removed))
    
    # Stop if very few points removed
    if(removed < 0.001 * n_before) break
  }
  return(data)
}

chl_22_qc_it <- clean_fluorometry(chl_22)
```
...
chl_22_q2_it %>% 
  ggplot(aes(measurementTime_2, fl)) +
  geom_line() +
  geom_point(data = filter(chl_22_q2_it, sd_mean_all > 4), color = "red") +
  geom_vline(xintercept = lubridate::ymd_hms("2022-05-16 00:12:00"), color = "red",
             size = 2) +
  labs(y = "Fluorescence") +
  theme_bw() +
...
```{r}
compare_og <- chl_22_q3 %>% 
  select(date_corr, fl_og = fl_med_day) %>% 
  distinct()

compare_it <- chl_22_q3_it %>% 
  select(date_corr, fl_it = fl_med_day) %>% 
  distinct()

compare <- compare_og %>% 
  left_join(compare_it)
```
```{r}
compare %>% 
  ggplot(aes(x = fl_og, y = fl_it)) +
  geom_point()
```
