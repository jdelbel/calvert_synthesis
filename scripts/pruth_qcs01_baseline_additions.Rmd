...
ctd <- read.csv(here("files_big", "8_binAvg-1736884493539.csv"))

#For some reason the portal cut out the ctd serial number column from the main csv, so porting it in here.
meta <- read.csv(here("files_big", "ctd-meta-1736884509824.csv"))

#Adding QCS07 data.
ctd_q7 <- read.csv(here("files_big", "qcs07_data.csv"))

#For some reason the portal cut out the ctd serial number column from the main csv, so porting it in here.
meta_q7 <- read.csv(here("files_big", "qcs07_meta.csv"))
...
```{r}
ctd <- rbind(ctd, ctd_q7)

meta <- rbind(meta, meta_q7)
```
...
chl_sf <- chl %>% 
  filter(site_id == "PRUTH" | site_id == "QCS01") %>% 
  filter(!is.na(chla)) %>%
  filter(chla > 0) %>% 
...
prof_join %>% 
  ...
  ggplot(aes(x = sum_chl_avg, y = flu_avg, color = station)) +
  geom_point() +
  facet_wrap(~ctdNum)
```
...
prof_ds <- prof %>% 
  filter(station == "QCS01" | station == "QCS07")

prof_ds <- rbind(prof_ds, prof_match)
```
...
```{r}
prof_red_pruth <- prof_ds %>% 
  filter(station == "PRUTH") %>% 
  group_by(castpk) %>%
  mutate(cast_num = cur_group_id()) %>% 
ungroup()

prof_red_qcs <- prof_ds %>% 
  filter(station == "QCS01") %>% 
  group_by(castpk) %>%
  mutate(cast_num = cur_group_id()) %>% 
ungroup()

prof_red_qcs7 <- prof_ds %>% 
  filter(station == "QCS07") %>% 
  group_by(castpk) %>%
  mutate(cast_num = cur_group_id()) %>% 
ungroup()
```
...
#Printing for easier visualization - just make sure to re-name with appropriate numbers
ggsave(here("figures", "prof_check_25-56_qcs07.png"),
       width = 16, height = 16, dpi = 300)
...
```{r}
write.csv(flu, here("outputs", "kc10_fzh01_dfo2_pruth_profs_qcs_qcs7_qc1_2025-01-14.csv"))
```
