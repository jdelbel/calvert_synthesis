...
chl <- read_csv(here("files", "2025-05-16.1hourSamples.2025.csv"))
...
chl_25 <- chl %>% 
  filter(year == 2025) %>% 
  select(measurementTime:year, fl = Chlorophyll_Avg) %>% 
...
chl_25 %>% 
  ...
  geom_line() +
  geom_point(data = filter(chl_25, gf < -1), color = "red")
```
...
# f3 <- 
  ...
  ggplot(aes(measurementTime_2, fl)) +
  geom_line() +
  geom_point(data = filter(chl_25_q2, sd_mean_all > 3), color = "red") +
...
chl_25_q4 %>%
  left_join(cd) %>% 
  ggplot() +
  geom_line(aes(date_corr, fl_med_day), size = 2) +
...
```{r}
write_csv(chl_25_q4, here("outputs", "buoy_corr_dm_2025.csv"))
```
