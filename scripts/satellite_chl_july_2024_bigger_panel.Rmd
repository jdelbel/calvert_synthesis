...
s <- read.csv(here("outputs", "sentinel_2024-07-19_bigger.csv"))

BC.shp <- readOGR(here("data","2_Shapefile", "COAST_TEST2.shp"))

ctd <- read.csv(here("files", "ctd_2024-07_Calvert.csv"))

f_cf <- read.csv(here("outputs", "flu_full_correct_2024.csv"))
...
Ncalvert <- extent(-128.29298, -127.54648, 51.32760, 52.07418)
...
ctd_coord <- arrange(mutate(ctd_coord,
...
ggplot() +
  ...
  coord_cartesian(xlim = c(-128.29298, -127.54648), ylim = c(51.32760, 52.07418)) +
  coord_fixed() +
  annotate("text", x = -128.15, y = 51.35, label = "July 19th, 2024",
           color = "white", size  = 15) +
...
ggsave(here("figures", "map_test_single_fc1_viridis_4.png"),
...
ctd %>% 
  filter(pres < 31) %>% 
  ggplot() +
  geom_line(aes(x = f, y = pres, color = station), orientation = "y", size = 4) +
  scale_y_reverse() +
  ggsci::scale_color_npg() +
...
ggsave(here("figures", "map_test_fc1.png"), fig,
        width = 16, height = 16, dpi = 300)
```
```{r}
ctd %>% 
